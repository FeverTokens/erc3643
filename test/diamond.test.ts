...
const deploy = async () => {
    ...
    const diamondAddress = await deployDiamond(cut);
    const diamondCutFacetAbi = JSON.parse(readFileSync(join(__dirname, '../artifacts/contracts/facets/DiamondCutFacet.sol/DiamondCutFacet.json'), 'utf8'));
    const diamondLoupeFacetAbi = JSON.parse(readFileSync(join(__dirname, '../artifacts/contracts/facets/DiamondLoupeFacet.sol/DiamondLoupeFacet.json'), 'utf8'));
    const ownershipFacetAbi = JSON.parse(readFileSync(join(__dirname, '../artifacts/contracts/facets/OwnershipFacet.sol/OwnershipFacet.json'), 'utf8'));
    ...
    return { diamondAddress, diamondCutFacet, diamondLoupeFacet, ownershipFacet, erc3643Facet };
};
...
describe('DiamondTest', function () {
    ...
    before(async function () {
        const { diamondCutFacet: diamondCutFacetValue, diamondLoupeFacet: diamondLoupeFacetValue, ownershipFacet: ownershipFacetValue } = await loadFixture(deploy);
    ...
    it('should add a new facet successfully', async function () {
        ...
        // }
        const { erc3643Facet, diamondLoupeFacet } = await loadFixture(deploy);
        // const diamondCutFacet = await hre.viem.deployContract("DiamondCutFacet", []);
        ...
        const newFacetAbi = JSON.parse(readFileSync(join(__dirname, '../artifacts/contracts/facets/erc3643Package/ERC3643Facet.sol/ERC3643Facet.json'), 'utf8')).abi;
    ...
    it('should check if a facet exists', async function () {
        ...
        const facetExists = facets.some((facet: any )=> facet.facetAddress === facetAddressToCheck);
