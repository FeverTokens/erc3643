...
abstract contract TrustedIssuersRegistryInternal is ITrustedIssuersRegistryInternal, AccessControlInternal {
    using TrustedIssuersRegistryStorage for TrustedIssuersRegistryStorage.Layout;
    
    bytes32 internal constant OWNER_ROLE = keccak256("OWNER_ROLE");

    function _addTrustedIssuer(address _trustedIssuer, uint256[] calldata _claimTopics) internal onlyRole(OWNER_ROLE) {
        TrustedIssuersRegistryStorage.Layout storage l = TrustedIssuersRegistryStorage.layout();
        ...
        for (uint256 i = 0; i < _claimTopics.length; i++) {
            ...
            l.issuerTopicIndex[topic][_trustedIssuer] = l.issuersForClaimTopic[topic].length;
    ...
    function _removeTrustedIssuer(address _trustedIssuer) internal onlyRole(OWNER_ROLE) {
        TrustedIssuersRegistryStorage.Layout storage l = TrustedIssuersRegistryStorage.layout();
        ...
        for (uint256 i = 0; i < topics.length; i++) {
            ...
            if (topicIndex != topicLastIndex) {
                address lastIssuer = l.issuersForClaimTopic[topic][topicLastIndex];
    ...
    function _updateIssuerClaimTopics(address _trustedIssuer, uint256[] calldata _claimTopics) internal onlyRole(OWNER_ROLE) {
        TrustedIssuersRegistryStorage.Layout storage l = TrustedIssuersRegistryStorage.layout();
        ...
        uint256[] memory oldTopics = l.trustedIssuers[_trustedIssuer].claimTopics;
    ...
    function _getTrustedIssuerClaimTopics(address _trustedIssuer) internal view returns (uint256[] memory) {
        TrustedIssuersRegistryStorage.Layout storage l = TrustedIssuersRegistryStorage.layout();
        return l.trustedIssuers[_trustedIssuer].claimTopics;
    }

    function _getTrustedIssuersForClaimTopic(uint256 claimTopic) internal view returns (address[] memory) {
        TrustedIssuersRegistryStorage.Layout storage l = TrustedIssuersRegistryStorage.layout();
        return l.issuersForClaimTopic[claimTopic];
    }

    function _hasClaimTopic(address _issuer, uint256 _claimTopic) internal view returns (bool) {
        TrustedIssuersRegistryStorage.Layout storage l = TrustedIssuersRegistryStorage.layout();
