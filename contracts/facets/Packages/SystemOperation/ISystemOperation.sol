...
interface ISystemOperation is ISystemOperationInternal{
    ...
    function isTransferCompliant(address _from, address _to) external view returns (bool);

    // Batch updates the frozen status of multiple addresses
    function batchUpdateFrozenStatus(address[] calldata _addresses, bool[] calldata _statuses) external; 

    // Batch updates the identity verification status of multiple addresses
    function batchUpdateIdentityVerification(address[] calldata _addresses, bool[] calldata _verificationStatuses) external;
